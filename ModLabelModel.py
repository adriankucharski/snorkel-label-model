...
class LabelModelFixed(nn.Module, BaseLabeler):
    ...
    **kwargs
    ...
    def _generate_O(self, L: np.ndarray, higher_order: bool = False) -> None:
        ...
        self.O = (
            torch.from_numpy(L_aug.T @ L_aug /
                             self.n).float().to(self.config.device)
        )
    ...
    def _init_params(self) -> None:
        ...
        for i in range(self.m):
            for y in range(self.cardinality):
                ...
                mu_init = torch.clamp(
                    lps[idx] * self._prec_init[i] / self.p[y], 0, 1)
    ...
    def get_weights(self) -> np.ndarray:
        ...
        for i in range(self.m):
            accs[i] = np.diag(cprobs[i, 1:, :] @
                              self.P.cpu().detach().numpy()).sum()
    ...
    def predict(
        ...
        array([0, 1, 0])
        """
        return super(LabelModelFixed, self).predict(L, return_probs, tie_break_policy)
    ...
    def score(
        ...
        {'f1': 0.8}
        """
        return super(LabelModelFixed, self).score(L, Y, metrics, tie_break_policy)
    ...
    def _loss_mu(self, l2: float = 0) -> torch.Tensor:
        ...
        """
        loss_1 = torch.norm((self.O - self.mu @ self.P @
                             self.mu.t())[self.mask]) ** 2
        loss_2 = torch.norm(
    ...
    def _set_class_balance(
        ...
        self.P = torch.diag(torch.from_numpy(
            self.p)).float().to(self.config.device)

    def _set_constants(self, L: np.ndarray) -> None:
        self.n, self.m = L.shape
        if self.m < 3:
            raise ValueError(
                "L_train should have at least 3 labeling functions")
        self.t = 1
    ...
    def _execute_logging(self, loss: torch.Tensor) -> Metrics:
        ...
        metrics_dict = {
            "train/loss": self.running_loss / self.running_examples}
    ...
    def _set_lr_scheduler(self) -> None:
        ...
        if lr_scheduler_name == "constant":
            ...
            def linear_decay_func(x): return (total_steps - self.warmup_steps - x) / (
    ...
    def _set_warmup_scheduler(self) -> None:
        ...
        if self.train_config.lr_scheduler_config.warmup_steps:
            ...
            def linear_warmup_func(x): return x / self.warmup_steps
            ...
            self.warmup_steps = int(
                warmup_percentage * self.train_config.n_epochs)
    ...
    def _fit_modified(
        ...
        **kwargs
        ...
        # My code #1.1 Compute class balance, this is constant for the every epoch
        if continue_training == False:
    ...
    def _eval_modified(self):
        self._clamp_params()
        self._break_col_permutation_symmetry()
        self.eval()

    def _calculate_size(self, images: List[List[np.ndarray]], num_of_lf: int) -> int:
        size = 0
        for i in range(len(images)):
            for k in range(len(images[i])):
                size += num_of_lf * images[i][k].size
        return size

    def _label_image(self, image: List[np.ndarray], labeling_functions: List[Callable]):
        labels = []
        for i in range(len(image)):
            filters = image[i]
            for lb_func in labeling_functions:
                label = lb_func(filters.flatten())
                labels.append(label)
        return labels

    def _label_images(self, images: List[List[np.ndarray]], labeling_functions: List[Callable]):
        all_labels = []
        for i in range(len(images)):
            labels = self._label_image(images[i], labeling_functions)
            all_labels.append(labels)
        return all_labels

    def run_model(self, array_data: List[List[np.ndarray]],
                  labeling_functions: List[Callable], batch_split=128,
                  Y_dev: np.ndarray = None, class_balance=[0.9, 0.1],
                  logs=False):

        if logs:
            print('Create space for dataset')
        size = self._calculate_size(array_data, len(labeling_functions))
        shape = (size // len(array_data[0]) // len(labeling_functions),
                 len(array_data[0]) * len(labeling_functions))
        dataset: np.ndarray = np.zeros((size), dtype='float16').reshape(shape)

        if logs:
            print('Labeling functions is running')
        last_index = 0
        all_labels = self._label_images(array_data, labeling_functions)

        for labels in all_labels:
            transpose = np.array(labels).T
            dataset[last_index:last_index + transpose.shape[0], :] = transpose
            last_index += transpose.shape[0]

        if logs:
            print('Training on batch is running')
        batch_size = dataset.shape[0] // batch_split
        np.random.shuffle(dataset)
        for i in range(0, dataset.shape[0], batch_size):
            print(
                f'Batch {i // batch_size} of {dataset.shape[0] // batch_size}' + (' ' * 20), end='\r')
            chunk = dataset[i:i + batch_size, ]
            self._fit_modified(chunk, seed=12344, log_freq=1, n_epochs=100,
                               Y_dev=Y_dev, class_balance=class_balance, continue_training=(i != 0))
        self._eval_modified()
        print('Completed', ' ' * 20)

    def predict_image(self, image: List[np.ndarray], labeling_functions: List[Callable], return_probs=False) -> Union[np.ndarray, Tuple[np.ndarray, np.ndarray]]:
        labels = self._label_image(image, labeling_functions)
        transpose = np.array(labels).T
        prediction = self.predict(transpose, return_probs)
        return prediction
